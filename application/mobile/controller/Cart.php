...
//        dump($result);
        // 从数据库查询这个用户等级
        $userLevel = M('users')->field('level')->where(['user_id'=>$this->user_id])->find();
     
        //  根据用户等级去用户登记表取出相对性的折扣
        $userDiscount = M('user_level')->field('discount')->where('level_id',$userLevel['level'])->find();
        // echo $userDiscount['discount'];
...
//        echo $result['total_price']['total_fee'] * $userDiscount['discount'];

        // $this->assign('couponList', $couponList); // 优惠券列表
        $this->assign('shippingList', $shippingList); // 物流公司
        $this->assign('cartList', $result['cartList']); // 购物车的商品
        $this->assign('userDiscount', $userDiscount ); // 获取商品折扣
